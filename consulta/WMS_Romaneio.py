...
# (Este é o seu código original, sem alterações)
# ===================================================================
...
def associar_separador():
    """
    Endpoint para associar um separador (CdUsrSep) a todos os itens
    de um NrRomaneio.
    """
    connection = None
    try:
        ...
        if not nr_romaneio or id_separador is None: # Permite ID 0, mas não None
        ...
        cursor.execute(sql_update, (id_separador, nr_romaneio))
        
        # Se nenhuma linha foi afetada (ex: romaneio não existe), avisa o usuário
        if cursor.rowcount == 0:
             return jsonify({"error": f"Nenhum registro encontrado para o Romaneio {nr_romaneio}. Nada foi atualizado."}), 404

        # Salva a alteração no banco de dados
        connection.commit()

        print(f"✅ Romaneio {nr_romaneio} associado ao usuário {id_separador} com sucesso.")
        return jsonify({"message": f"Romaneio {nr_romaneio} associado ao usuário {id_separador} com sucesso. Linhas afetadas: {cursor.rowcount}"}), 200

    except Exception as e:
        if connection:
            connection.rollback() # Desfaz a alteração se der erro
        print(f"❌ Erro ao associar separador: {e}")
        return jsonify({"error": str(e)}), 500
